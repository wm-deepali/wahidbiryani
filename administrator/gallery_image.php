<?php 
include("header.php");
$query = "SELECT id,title FROM bk_gallery_category WHERE status='active'";
$get_cat=mysqli_query($db->conn,$query) or die(mysqli_connect_errno()."Data cannot inserted");
if($_REQUEST['subview']=="add")
 $_HEADING = "Add New Gallery";
else if($_REQUEST['subview']=="update")
{
 $_HEADING = "Update Gallery";   
 $query = "SELECT * FROM bk_gallery_image WHERE id='".$_REQUEST['id']."'";
...
{
 $_HEADING = "View All Gallery";
 $rowsperpage = 2000;
 if($_REQUEST['status']=="all" || $_REQUEST['status']=="")
   $sql1 = "SELECT * FROM bk_gallery_image ORDER BY id DESC ";
 elseif($_REQUEST['status']!="all" && $_REQUEST['status']!="")
   $sql1 = "SELECT * FROM bk_gallery_image WHERE status = '".$_REQUEST['status']."' ORDER BY id DESC ";
 $db1 = $db->conn;
   ...
   if($_REQUEST['status']=="all" || $_REQUEST['status']=="")
     $sql2 = "SELECT * FROM bk_gallery_image ORDER BY id DESC LIMIT " . $pagination->getLimit() . ", " . $rowsperpage; 
   elseif($_REQUEST['status']!="all" && $_REQUEST['status']!="")
    $sql2 = "SELECT * FROM bk_gallery_image WHERE status = '".$_REQUEST['status']."' ORDER BY id DESC LIMIT " . $pagination->getLimit() . ", " . $rowsperpage; 
...
<div class="content-wrapper">
  ...
  <section class="content-header">
   ...
   <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item"><a href="#"><i class="fa fa-files-o"></i> Our Menu Management</a></li>
    <li class="breadcrumb-item active">Edit Gallery</li>
  </ol>
  <?php if($_SESSION['image']!=""){ ?>
  <div class="alert alert-success alert-dismissable">
    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
    <?php echo $_SESSION['image'];$_SESSION['image']="";?>
  </div>
  <?php } ?>
</section>
<br>
   ...
   <div class="table-rs">
    ...
    <tbody>
     ...
     <tr>
      <td><?php echo $i?></td>
      <?php 
      $query = "SELECT id,title FROM bk_gallery_category WHERE status='active' and id='".$row['cat_id']."'";
      $get_cat=mysqli_query($db->conn,$query) or die(mysqli_connect_errno()."Data cannot inserted");
      ...
      <td>
        <ul class="actions">
          ...
          <li><a href="action/gallery_image.php?subview=delete&id=<?php echo $row['id'];?>" onclick="return confirm('Are you sure?')"><i class="fa fa-trash"></i></a></li>
        </ul>
      </td>
    </tr>
    <div id="<?php echo $row['id'];?>" class="modal fade" role="dialog">
      <div class="modal-dialog">
        ...
        <div class="modal-content">
          ...
          <form action="action/gallery_image.php?subview=update&id=<?php echo $row['id'];?>" method="post" enctype="multipart/form-data">
           ...
           <div class="form-group">
             <div class="row">
              <div class="col-sm-12">
                ...
                <div class="controls">
                  <?php  $query = "SELECT id,title FROM bk_gallery_category WHERE status='active'";
                  $get_cat=mysqli_query($db->conn,$query) or die(mysqli_connect_errno()."Data cannot inserted"); 
                  ?>
         ...
         <div class="col-sm-12">
              ...
              <img src="gallery/<?php echo $row['image_name']?>" height="100" width="100"/>
                <?php } ?>
                    ...
                    <div class="form-group">
                      <label for="discount">Image only upload(255*170)</label>
...
<div id="add_gal" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <form action="action/gallery_image.php?subview=add" method="post" enctype="multipart/form-data">
        <div class="modal-body">
         <div class="form-group">
           <div class="row">
            <div class="col-sm-12">
          ...
          <div class="col-sm-12">
                <h5>Featured Image upload(255*170)<span class="text-danger">*</span></h5>
                <div class="controls">
