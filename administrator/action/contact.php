...
if($_REQUEST['subview']=="add")
   {
      ...
      if($db->insert($data,'bk_contact') == true)  
      {
    ...
    if($check !== false) {
         ...
         /*  $exp = explode(".",$image);
          ...
          if($upload)
            ...
            $sql2 = "UPDATE bk_contact SET image_name='".$image_name."' WHERE id='".$last_id."'";
      ...
      echo "<script>window.location.href='../contact.php?subview=list'</script>"; 
     }  
      ...
      echo "<script>window.location.href='../contact.php?subview=add'</script>";  
      } 
  ...
  if($_REQUEST['subview']=="update")
   ...
   $field ="id";   
   $id = $_REQUEST['id'];
      if($db->update($data,'bk_contact',$field,$id) == true)  
      {
       if($_FILES['image']['name']!="")
         {
          $check = getimagesize($_FILES["image"]["tmp_name"]);
    if($check !== false) {
          ...
          if($upload)
            {
              $thumbnail = $obj->generateThumbnail("../gallery/".$image_name, "../gallery/thumb/".$image_name, 100, 100);
            $sql2 = "UPDATE bk_contact SET image_name='".$image_name."' WHERE id='".$id."'";
      ...
      //exit;
    echo "<script>window.location.href='../contact.php?subview=update&id=".$id."'</script>";        
   } 
