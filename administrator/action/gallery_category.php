...
if($_REQUEST['subview']=="add")
   {
		 $title = mysqli_real_escape_string($db->conn,$_POST['title']);
		  ...
		  if($db->insert($data,'bk_gallery_category') == true)  
			{
			$last_id = mysqli_insert_id($db->conn);
			
			$_SESSION['category'] ="Category Added Success Fully.";
			echo "<script>window.location.href='../gallery_category.php?subview=list'</script>"; 	  
  ...
  if($_REQUEST['subview']=="update")
   {
    $title = mysqli_real_escape_string($db->conn,$_POST['title']);
	 ...
	 $field ="id";	 
	 $id = $_REQUEST['id'];
	 if($db->update($data,'bk_gallery_category',$field,$id) == true)  {
  ...
  else if($_REQUEST['subview']=="delete")
   {
	   ...
	   $id = $_REQUEST['id'];
	   //print_r($_POST['id']);
	    $sql2 = "SELECT * FROM bk_gallery_category WHERE id='".$id."'";
