...
else if($_REQUEST['subview']=="update")
{
  ...
  $query = "SELECT * FROM bk_category WHERE id='".$_REQUEST['id']."'";
...
else if($_REQUEST['subview']=="list")
{
 ...
 if($_REQUEST['status']=="all" || $_REQUEST['status']=="")
   $sql1 = "SELECT * FROM bk_category ORDER BY id DESC ";
 elseif($_REQUEST['status']!="all" && $_REQUEST['status']!="")
   $sql1 = "SELECT * FROM bk_category WHERE status = '".$_REQUEST['status']."' ORDER BY id DESC ";
 $db1 = $db->conn;
   ...
   if($_REQUEST['status']=="all" || $_REQUEST['status']=="")
     $sql2 = "SELECT * FROM bk_category ORDER BY id DESC LIMIT " . $pagination->getLimit() . ", " . $rowsperpage; 
   elseif($_REQUEST['status']!="all" && $_REQUEST['status']!="")
    $sql2 = "SELECT * FROM bk_category WHERE status = '".$_REQUEST['status']."' ORDER BY id DESC LIMIT " . $pagination->getLimit() . ", " . $rowsperpage; 
 ...
 <!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <li class="breadcrumb-item"><a href="#"><i class="fa fa-files-o"></i> Category Management</a></li>
            ...
            <td>
                        <ul class="actions">
                          <li><a href="?subview=update&id=<?php echo $row['id'];?>" data-target="#<?php echo $row['id'];?>" data-toggle="modal" title="Edit"><i class="fa fa-edit" aria-hidden="true"></i></a></li>
                            <li><a href="action/category.php?subview=delete&id=<?php echo $row['id'];?>" title="Delete Gallery" onclick="return confirm('Are You Sure?')"><i class="fa fa-trash" aria-hidden="true"></i></a></li>
                        </ul>
                       </td>
          </tr>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
       ...
       <form action="action/category.php?subview=update&id=<?php echo $row['id'];?>" method="post" enctype="multipart/form-data">
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <form action="action/category.php?subview=add" method="post" enctype="multipart/form-data">
      <div class="modal-body">
       <div class="form-group">
           <div class="row">
            <div class="col-sm-12">
